...
typedef struct {
    ...
    int  health;
    int  keys;
    char symbol;      /* '1', '2', or '3' */

    /* Extended fields: per-player statistics (Part B) */
    int movesMade;
    int treasuresFound;
    int trapsTriggered;
    int damageTaken;
...
int    hiddenTrap[GRID_SIZE][GRID_SIZE];   /* 1 = trap present, 0 = none  */

Player players[MAX_PLAYERS];
int    playerCount = 2;

LogEntry eventLog[LOG_SIZE];
int      logCount = 0;      /* total entries ever added (keeps growing)   */
...
/*  INPUT HELPERS                                                          */
/* ---------------------------------------------------------------------- */
int inputEOF = 0; /* set to 1 once stdin is closed/exhausted */
...
void readLine(char *buf, int size) {
    ...
    if (len > 0 && buf[len - 1] == '\n') {
        ...
        /* input longer than buffer - clear remaining chars */
        flushStdin();
    }
}
...
/* Randomly places `count` copies of `symbol` on free tiles. Returns
   number actually placed (useful for traps where we still return count). */
int placeRandomSymbol(char symbol) {
    /* not used directly - kept for potential reuse */
    (void)symbol;
    return 0;
}
...
void printMap(void) {
    ...
    /* column header - use a fixed 3-char field so single- and double-digit
       column indices both line up cleanly with the 3-char cells below */
    printf("    ");
...
/* Applies trap -> treasure -> health -> key effects at player's tile */
void processTile(int index) {
